...
#5. Request a node
Request one core from the ficklin_class partition which will allow us to run commands on one of the head node.  
Run on the command line: idev --partition=ficklin_class --account=ficklin_class -t 12:00:00  
We will  combine the files in output_results.txt with the command:  
